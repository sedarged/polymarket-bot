...
export function FillsTable() {
  ...
  const fills = data?.fills?.slice(-20).reverse() ?? []
  ...
  return (
    <div className="rounded-lg border border-border bg-card">
      ...
      {error ? (
        <div className="p-5">
          <ErrorState message="Failed to load fills" onRetry={() => mutate()} />
        </div>
      ) : !data ? (
        <div className="p-5">
        ...
        <EmptyState
          ...
          title="No fills yet"
          description="Trade executions will be recorded here."
        />
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-5 py-2.5 text-right text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Fee</th>
                <th className="px-5 py-2.5 text-right text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Time</th>
              </tr>
            </thead>
            <tbody>
              {fills.map((f, i) => (
                ...
                >
                  <td className="px-5 py-3 font-mono text-xs text-foreground">{f.orderId.slice(0, 8)}...</td>
                  <td className="px-5 py-3 font-mono text-xs text-muted-foreground">{f.tokenId.slice(0, 10)}...</td>
                  <td className={cn("px-5 py-3 text-xs font-medium", f.side === "BUY" ? "text-success" : "text-danger")}>{f.side}</td>
                  <td className="px-5 py-3 text-right font-mono text-xs text-foreground">{formatNumber(f.price, 4)}</td>
                  <td className="px-5 py-3 text-right font-mono text-xs text-foreground">{formatNumber(f.size)}</td>
                  <td className="px-5 py-3 text-right font-mono text-xs text-muted-foreground">{f.fee ? formatNumber(f.fee, 4) : "-"}</td>
                  <td className="px-5 py-3 text-right text-[11px] text-muted-foreground">{formatTimestamp(f.timestamp)}</td>
                </tr>
              ))}
            </tbody>
          </table>
