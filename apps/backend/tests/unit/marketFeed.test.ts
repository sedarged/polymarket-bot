...
// Mock CLOB client so "uncached orderbook" resync does not hit the real API (avoids 404)
...
describe('MarketFeedClient', () => {
  ...
  describe('edge cases', () => {
    ...
    it('should handle price change for uncached orderbook gracefully', async () => {
      ...
      // Should not throw; resync fetches from mocked CLOB and caches
      server.clients.forEach((ws) => {
        ws.send(JSON.stringify(priceChange));
      });

      // Give resync time to complete
      await new Promise((resolve) => setTimeout(resolve, 200));

      const orderbook = client.getOrderbook('uncached-token');
      expect(orderbook).not.toBeNull();
      expect(orderbook?.asset_id).toBe('uncached-token');
    });
