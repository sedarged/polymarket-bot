...
describe('UserFeedClient', () => {
  ...
  describe('connection', () => {
    // Skip on some environments where local WS server connection never completes (timeout)
    it.skip('should connect to WebSocket and emit connected event', async () => {
      ...
      // Use 127.0.0.1 to avoid IPv6 ::1 connection refused on some environments
      client = new UserFeedClient({
        wallet: testWallet,
        wsUrl: `ws://127.0.0.1:${port}/ws/user`, // Use wsUrl parameter for testing
      });
  ...
  describe('error handling', () => {
    it('should emit error event on WebSocket error', async () => {
      ...
      client = new UserFeedClient({
        wsUrl: `ws://127.0.0.1:${port}/ws/user`,
        wallet: testWallet,
      });

      client.on('error', errorHandler);

      // Close server so connection fails
      server.close();
      await new Promise((resolve) => setTimeout(resolve, 100));

      // connect() promise rejects on error; do not await so we can wait for 'error' event
      client.connect().catch(() => {});

      // Wait for client to emit 'error' (connection refused)
      await new Promise<void>((resolve) => {
