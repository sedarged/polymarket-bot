...
/**
 * Type-safe helper to pass UserOrderWithClientId to SDK functions.
 * 
 * This function provides a documented type boundary for passing our extended
 ...
 * 2. TypeScript's structural typing allows passing a more specific type to a function
 *    expecting a less specific type
 * 3. The CLOB API explicitly accepts and uses clientOrderId for idempotency
 * 4. The SDK forwards the object to the API without validating/rejecting extra fields
 * 5. Verified against CLOB REST API docs and tested with @polymarket/clob-client v5.2.1
 * 
 * Runtime behavior:
 ...
 * 
 * Type safety:
 * - Input is validated to match UserOrderWithClientId structure at compile time
 * - Output is properly typed as UserOrder for SDK consumption
 * - No type assertions needed - structural subtyping handles this naturally
 * 
 * Maintenance:
...
function toUserOrder(orderWithClientId: UserOrderWithClientId): UserOrder {
  ...
  // TypeScript's structural typing allows passing a more specific type naturally.
  return orderWithClientId;
}
  ...
  async createOrdersBatch(orders: Array<{
    ...
    try {
      ...
      for (const prepared of preparedOrders) {
        try {
          // Create signed order using SDK with type-safe helper
